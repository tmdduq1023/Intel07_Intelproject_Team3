...
def cls_loss_and_acc(logits_list, target, ignore_index=-1):
    ...
    for k, lg in enumerate(logits_list):
        ...
        loss = F.cross_entropy(lg, t, ignore_index=ignore_index)
        total_loss += loss
...
def train_one_epoch(model, loader, optim, device, lambda_reg=1.0, lambda_cls=1.0, scaler=None, reg_std=None):
    model.train()
...
def main():
    ...
    ap.add_argument("--out-dir",    type=str, default="./runs/skin_roi_v2_1")
    ...
    ap.add_argument("--auto-reg-std", action="store_true", help="train json에서 회귀 통계를 자동 산출하여 표준화 적용")
    args = ap.parse_args()

    print("
[INFO] Training with the following parameters:")
    print("----------------------------------------")
    ...
    if args.auto_reg_std:
        print("
[INFO] Calculating regression statistics from training data...")
        ...
        print("
[INFO] Regression standardization is not used.")
...
[INFO] Training started at: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    for ep in range(1, args.epochs+1):
        tr_loss, tr_mae, tr_acc = train_one_epoch(model, train_loader, optim, device,
                                                  lambda_reg=args.lambda_reg, lambda_cls=args.lambda_cls, scaler=scaler, reg_std=reg_std)
...
class RoiDataset(Dataset):
    ...
    def __init__(self, json_path, data_root, roi_label_space, img_size=384, is_train=True, pad_ratio=0.10, reg_std=None):
        ...
        self.roi_cls_maps = roi_label_space["class_key_to_index"]
        self.pad_ratio = pad_ratio
        self.reg_std = reg_std or {} # --- [추가] reg_std 저장
...
def train_one_epoch(model, loader, optim, device, lambda_reg=1.0, lambda_cls=1.0, scaler=None, reg_std=None):
    ...
    for x, reg_t, cls_t, metas in pbar:
        ...
        with autocast(device_type="cuda", enabled=(scaler is not None)):
            ...
            for i in range(x.size(0)):
                roi = roi_names[i]
                # --- [변경] 회귀 로직: Huber Loss 사용 및 MAE 역변환 계산 ---
                reg_pred = out_reg[i]
                if reg_pred is not None and reg_pred.size(1) > 0:
                    target_slice = reg_t[i, :reg_pred.size(1)]
                    huber_loss = masked_huber_loss(reg_pred.squeeze(0), target_slice)
    ...
    for x, reg_t, cls_t, metas in pbar:
        ...
        for i in range(x.size(0)):
            ...
            if reg_pred is not None and reg_pred.size(1) > 0:
                ...
                huber_loss = masked_huber_loss(reg_pred.squeeze(0), target_slice)
                if huber_loss is not None:
                    total_loss = total_loss + huber_loss # lambda_reg는 train에만
...
def main():
    ...
    for ep in range(1, args.epochs+1):
        ...
        if improved:
            torch.save({"model": model.state_dict(),
                        "roi_label_space": roi_label_space,
                        "reg_std": reg_std, # --- [추가] 체크포인트에 reg_std 저장
                        "epoch": ep,
                        "val_mae": vl_mae,
                        "val_acc": vl_acc,
                        "score": score}, best_path)
